...
#   wget -qO- https://raw.githubusercontent.com/rios0rios0/ccswitch/main/install.sh | sh
#
# Options:
...
#   CCSWITCH_INSTALL_DIR   Installation directory (default: ~/.local/bin)
#   CCSWITCH_VERSION       Specific version to install (default: latest)
#   CCSWITCH_FORCE         Force installation (true/false, default: false)
#   CCSWITCH_DRY_RUN       Dry run mode (true/false, default: false)
#
...
INSTALL_DIR="${CCSWITCH_INSTALL_DIR:-$DEFAULT_INSTALL_DIR}"
VERSION="${CCSWITCH_VERSION:-latest}"
FORCE="${CCSWITCH_FORCE:-false}"
DRY_RUN="${CCSWITCH_DRY_RUN:-false}"
...
ENVIRONMENT VARIABLES:
    CCSWITCH_INSTALL_DIR   Installation directory
    CCSWITCH_VERSION       Specific version to install (default: latest)
    CCSWITCH_FORCE         Force installation (true/false)
    CCSWITCH_DRY_RUN       Dry run mode (true/false)
...
main() {
    ...
    info "  ${BINARY_NAME} enroll                 # capture the currently logged-in account"
    info "  # log in as another account with 'claude' + /login, then:"
    info "  ${BINARY_NAME} enroll                 # capture the next account"
    info "  ${BINARY_NAME} list                   # see all accounts and their usage"
    info "  ${BINARY_NAME} monitor --ensure-daemon  # start the background rotation daemon"
}
