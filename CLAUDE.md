...
`ccswitch` is a Go CLI + daemon that watches Claude Code usage and rotates
`~/.claude/.credentials.json` (the `claudeAiOauth` block) between enrolled backup accounts when the
active one is exhausted. The next `claude` launch then authenticates as the swapped-in account. See
`README.md` for user-facing behavior, flags, and shell integration.
