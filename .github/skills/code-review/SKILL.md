---
name: code-review
description: "Review pull requests and diffs in ccswitch — the Go daemon that rotates Claude Code accounts — against the rios0rios0/guide standards, with extra weight on the credential-store invariants, the six cross-compiled targets, and atomic 0600 persistence. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `ccswitch`

`ccswitch` monitors Claude Code usage and swaps the `claudeAiOauth` block of the credential store when the active account is exhausted. It reads and rewrites live credentials on Linux, macOS, and Windows — the invariants below exist because breaking one destroys a user's tokens.
...
## What matters most in `ccswitch`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **Match accounts by identity, never by refresh token.** The server rotates the refresh token on every refresh, so `OAuthCredentials.SameAccountAs` is a positive signal only — a false result does not mean "different account". Resolution goes through `Store.MatchAccount` (`accountUuid`/`email` first, refresh token only as a last resort). Getting this wrong pins the store to a rotated-away token and makes `ensure` clobber good credentials. **Critical.**
- **The macOS keychain write is a read-modify-write.** The `Claude Code-credentials` generic-password item holds `claudeAiOauth` *and* `mcpOAuth`. Merge into the stored document, refuse to write when it cannot be read or parsed, and read it back to confirm. Only `ErrKeychainItemNotFound` (exit status 44) means absence — treating any failed read as absence erases every authenticated MCP server's tokens. **Critical.**
- **`security -i` truncates command lines over `securityStdinLimit` (4032)**, which is why larger payloads go through argv. A change that routes a large payload back through stdin silently corrupts the keychain item.
- **Six targets ship, so platform code must compile for all of them.** OS-specific code belongs in `_unix.go` / `_darwin.go` / `_windows.go` pairs; nothing else branches on the OS. `make cross-compile` type-checks linux/darwin/windows × amd64/arm64 and the per-PR workflow runs the same matrix. `make lint` only covers the host, so a change to platform code needs `GOOS=darwin` and `GOOS=windows` runs too. Skipping this is how `Setsid` reached delivery and left every release up to `0.2.2` with no binaries.
- **All persistence is atomic and owner-only** — temp file plus rename, mode `0600`. A direct write to the store path, or a wider mode, is a Critical finding.
- **The Claude desktop app is not a Claude Code session.** Session detection must not count it; check `ProcSessionsRepository`, `PSSessionsRepository`, and `ToolhelpSessionsRepository` stay consistent about what counts.
- User-facing output goes to `os.Stdout`/`os.Stderr` with a `[ccswitch]` prefix; diagnostics go through Logrus aliased as `logger`. Do not mix the two.

### Commands a reviewer should be able to quote

```bash
make build          # bin/ccswitch
make lint           # golangci-lint (strict; see the pipelines .golangci.yml)
make test           # unit + integration
make cross-compile  # go vet across all six released OS/arch targets
make sast
GOOS=darwin make lint && GOOS=windows make lint   # after touching platform code
```
...
- Tests live in external `_test` packages, carry **no** build tags, and use `// given` / `// when` / `// then` blocks.
- No mocking library — doubles come from `test/doubles`, and HTTP adapters are exercised against a real `httptest.NewServer`.
- A change to credential matching, keychain merging, or session detection needs a test that would have failed before the change.
...
- The `_unix.go` / `_darwin.go` / `_windows.go` triplication — it is the agreed shape.
